...
> **⚠️ 重要更新（2025年）**：下面的代码基于旧版 API，请参考 [2.6 Sepolia FHEVM 正确配置](#26-sepolia-fhevm-正确配置) 获取最新 API。

```solidity
// contracts/DataMarketplaceFHE.sol

// ⚠️ 旧版 API（仅供参考）
// ✅ 新版应使用 SepoliaConfig（见 2.6 节）
contract DataMarketplaceFHE is 
    SepoliaConfig,  // ✅ 新版：来自 @fhevm/solidity/config/ZamaConfig.sol
    // ❌ 不再需要 GatewayCaller（改用公开解密）
    Ownable,
    ReentrancyGuard
{
    // ✅ 使用 FHE 类型
    ...
    // ✅ 新版 API：使用 externalEuint32（不是 einput）
    function uploadDataset(
        string memory name,
        string memory description,
        externalEuint32[] calldata encryptedValues,  // ✅ 新版 API
        bytes[] calldata inputProofs,
        uint256 pricePerQuery
    ) external returns (uint256) {
        for (uint256 i = 0; i < encryptedValues.length; i++) {
            // ✅ 新版：使用 FHE.fromExternal()
            encryptedData[i] = FHE.fromExternal(
                encryptedValues[i], 
                inputProofs[i]
            );
        }
    ...
    {
        euint32 sum = FHE.asEuint32(0);  // ✅ 新版：FHE（不是 TFHE）
        for (uint256 i = 0; i < data.length; i++) {
            sum = FHE.add(sum, data[i]);
        }
        return FHE.div(sum, uint32(data.length));
    }
    
    // ✅ 新版：使用公开解密（不是 Gateway 回调）
    function aggregateAndDecrypt(uint256[] calldata datasetIds) external {
        euint32 result = _computeMean(datasetIds);
        FHE.makePubliclyDecryptable(result);  // ✅ 前端可用 publicDecrypt
        // ...
    }
}
```

**📚 完整的新版配置请参考 [2.6 Sepolia FHEVM 正确配置](#26-sepolia-fhevm-正确配置) 章节。**

### 12.4 常见问题速查

| 问题 | 原因 | 解决方案 |
|------|------|---------|
| **❌ 旧版：TFHE.asEuint32 API 错误** | 需要 `einput` 和 `bytes` | ✅ **新版**：使用 `FHE.fromExternal(externalEuint32, proof)` |
| **FHE 函数不能 view** | FHE 操作修改状态 | 移除 `view` 关键字 |
| **Mock 和 FHE 模式冲突** | 自动网络检测 | 只由 `FHEVM_ENABLED` 控制 |
| **FHEVM SDK 初始化失败** | API 不匹配 | 优雅降级，返回 null |
| **❌ GatewayCaller not found** | 旧版 API，新版已移除 | ✅ **新版**：继承 `SepoliaConfig`，使用 `FHE.makePubliclyDecryptable()` |
| **❌ fhevm/gateway/GatewayCaller.sol not found** | 包结构变化 | ✅ **新版**：使用 `@fhevm/solidity`，不需要 GatewayCaller |
